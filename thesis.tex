...
a) diversification: algorithm should explore the search space
...
Template of an evolutionary algorithm. (b Talbi) \
Generate(P(0)) ; \
t=0; \
...
Mutation of individual usualy consists of randomly selecting some configuration from its neghbourhood as its ofspring. This step provides intensification for evolution heuristic.

Crosover usualy consists of creating an offspring by permutation of two individuals. This step provides diversification for evolution heuristic.
...
0) union of nondominated archive and current population is created. For each individual, algorithm computes
	a) strength based on number of individuals evaluated individual dominates
	b) raw fitness, that is determined by the sum of strengths of individuals that dominate the evaluated one \note{this criterion is to be minimized}
	c) density information
	d) agregated fittness
1) new archive of nondominated solutions is created from the union
2) if a) new archive has less individuals, than its desired capacity,
	best remaining individuals based on agregated fittness will ba added
  b) archive has more individuals than its desired capacity,
	remove some of them from the archive based on density information
...
1) additive $\epsilon$-indicator that quantifies the minimal distance first pareto set has to be moved in each dimension in objective space such that the second pareto set is weakly dominated.
...
1) it is an efficient glue language. 
...
2) it has good facilities for statistical analysis.
...
3) it is easily integrated with C++.
We strive to make the integration with ParadisEO simple and extensible. 

\section{Evaluation function}
To simplify configuration, inputs and outputs of evaluation function are allways a list of floating point numbers. We believe this is general enough, and that most WSN optimisation problems can be fitted to this constraint.
...
1) maximizing the fittness of result
...
Or we could use different convergence criterion. One of the more popular is maximizing the hypervolume of the objective space dominated by the resulting approximation
 [TODO]
...
3) treshold for minimal number of recieved packets for a node to be considered malicious
4) treshold for ratio between forwarded and recieved packets for a node to be considered to be malicious
...
$$M = \{n \in N|r_n \geq r_{min} \and \frac{f_n}{r_n} \geq d_{max} \} $$

And the set of benign nodes is:

$$B = \{n \in N|r_n < r_{min} \or \frac{f_n}{r_n} < d_{max} \} $$
